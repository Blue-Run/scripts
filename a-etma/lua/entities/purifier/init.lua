...
function ENT:StartTouch(ent)
	if ent:GetClass() == "raw_cocaine" and self.IsBaking == false then
		ent:Remove()
		self.NumGrapes = (self.NumGrapes or 0) + 1
		if self.NumGrapes >= self.NumGrapesRequired then
			self.IsBaking = true
			self.DoneBaking = CurTime() + math.random(65, 150) --math.random(60, 120)
			self.NumGrapes = 0
		end
	end
end
...
function ENT:Think()
	...
	if self.IsBaking == true then
		if self.DoneBaking <= CurTime() then
			self.IsBaking = false
			local Coke = ents.Create("pure_cocaine")
			Coke:SetPos(self:GetPos() + Vector(0, 0, 30))
			Coke:Spawn()
			self.DoneBaking = 0
			--self.Money = self.Money + 50 -- add money for producing Coke
		end
	end
end
...
function ENT:Think()
	...
	if self.IsBaking == true then
		if self.DoneBaking <= CurTime() then
			self.IsBaking = false
			local Coke = ents.Create("Coke")
			Coke:SetPos(self:GetPos() + Vector(0, 0, 40))
			Coke:Spawn()
			self.DoneBaking = 0
			--self.Money = self.Money + 50 -- add money for producing Coke
		end
