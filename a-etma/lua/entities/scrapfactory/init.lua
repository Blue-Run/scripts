...
function ENT:Initialize()
    ...
    self.SpawnChanceTable = {
        -- Change this table to set the spawn chance and entity to spawn
        {
            spawnChance = 80,
    ...
    if self.IsBaking == true then
        ...
        if self.DoneBaking <= CurTime() then
            ...
            if randomEntity then
                ...
                ent:Spawn()
                if randomEntity.OnSpawn then randomEntity.OnSpawn(ent) end
            end

            self.DoneBaking = 0
        end
    else --self.Money = self.Money + 50 -- add money for producing wine
