...
hook.Add("PostDrawOpaqueRenderables", "scrap_npc", function()
	for k, v in pairs (ents.FindByClass("scrap_npc")) do
		if (v:GetPos():Distance(LocalPlayer():GetPos()) < HR_CrystalMeth.DrawDistance) then
			...
			cam.Start3D2D(v:GetPos()+v:GetUp()*78, Angle(0, LocalPlayer():EyeAngles().y-90, 90), 0.075);
				draw.SimpleTextOutlined("Scrap Buyer", "HR_CR_BuyerFont", 0, 0, Color(255,255,255), TEXT_ALIGN_CENTER, TEXT_ALIGN_LEFT, 1, Color(25, 25, 25, 100));	
			cam.End3D2D()
		end
	end
end)